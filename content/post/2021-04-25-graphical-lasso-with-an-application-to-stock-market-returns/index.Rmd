...
This is the first post by student-w, a suitable pseudonym for a statistician. Note that these
blog posts will typically be written while I´m learning a particular topic, such that it should 
not be taken as an authoritarian source of truth, but suitably correlated with the truth. 
...
lasso, we can think of how we can enforce sparsity in this graph. This is exactly what the classical 
lasso regression by Tibshirani et. al does, by including a penalty on the $\beta$-parameters!
...
We have available price data on a selection of the companies listed on Oslo Stock Exchange. In the code below we read the daily price data for a selection of companies, and plot the movements. 
